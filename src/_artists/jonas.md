---
name: Jonas Bek
slug: jonas
sortOrder: 4
description: "Jonas Bek tatoverer traditional og neo-traditional — farver, stærke konturer, klassiske motiver. Walk-ins ok hos Ink & Art Copenhagen."
description_en: "Jonas Bek works in traditional and neo-traditional — colour, strong outlines, classic motifs. Walk-ins welcome at Ink & Art Copenhagen."
role:
  da: Tatovør
  en: Tattooer
styles: [traditional, neo-traditional, color]
instagram: jonas.bek.ink
booksysId: 4
languages: [da, en]
portrait:
  src: portrait.jpg
  alt:
    da: Jonas Bek bag tatover-stolen
    en: Jonas Bek behind the chair
portfolio: []
bio:
  da: |
    <p>Jonas elsker traditional og neo-traditional. Farver, stærke konturer, klassiske motiver — sailors, dolke, paneler.</p>
    <p>Walk-ins ok hvis der står en flash på væggen der råber dig op.</p>
  en: |
    <p>Jonas loves traditional and neo-traditional. Colour, strong outlines, classic motifs — sailors, daggers, panels.</p>
    <p>Walk-ins ok if a flash on the wall calls your name.</p>
---
