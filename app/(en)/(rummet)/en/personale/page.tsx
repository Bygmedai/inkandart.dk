...
import { loadTeamguideEn, loadKontakt, loadAabningstider, loadPiercingpriser } from "@/lib/content";
import { t } from "@/lib/i18n";
import { tiderListe } from "@/lib/tider";
import { VAGT_COOKIE, tokenErGyldigt } from "@/lib/vagt";

export const metadata: Metadata = {
  title: "Team guide · Ink & Art",
...
export default async function PersonalePageEn({
  searchParams,
}: {
  searchParams: Promise<Record<string, string | string[] | undefined>>;
}) {
  const params = await searchParams;
  const c = loadTeamguideEn();
  ...
  if (!aaben) {
    return (
      <RummetShell lang="en">
        <main id="main" lang="en" className="rum-room rum-legal">
          <section className="rum-vagt">
            ...
            <form method="post" action="/api/vagt" className="rum-tjek__form">
              <input type="hidden" name="retur" value="en/personale" />
  ...
  return (
    <RummetShell lang="en">
      <main id="main" lang="en">
        <TeamguideFlade
          c={c}
          tider={tiderListe(loadAabningstider(), t("en").rummet.tider)}
          kontakt={{
          ...
          priser={loadPiercingpriser("en").grupper}
          retur="en/personale"
          ordAf="of"
          ordNulstil="Reset"
        />
